...
def normalize_name(name: str) -> str:
    """Strip trailing periods and normalize whitespace/case (e.g., 'I.LIKELY.' -> 'I.LIKELY')."""
    if not isinstance(name, str):
        return name
    name = re.sub(r"\.+$", "", name)          # remove only trailing dots
...
# =========================
# MAIN LOOP (TD logic same; adds targets + receptions)
# =========================
for idx, r in pbp.iterrows():
    ...
    if r['ispass'] and not r['issack'] and not r['isincomplete'] and not r['isinterception']:
        ...
        if td_after:
            ...
            if rec_name:
                rec_name = normalize_name(rec_name)
                if 'FUMBLES' in d.upper() and 'RECOVERED BY' in d.upper():
                    fr = fumble_recovery_scorer(d)
                    if fr:
                        fr_name = normalize_name(fr[0])
                        if fr_name == rec_name and (fr[1] == off or fr[1] is None):
        ...
        if init_rec:
            init_rec = normalize_name(init_rec)
...
# dim_games  **ONLY CHANGE: iso_week calculation**
# dim_games  (only the iso_week logic is changed to Tuesday→Monday)
dim_games = (
    pbp[["gameid","gamedate"]]
      .dropna()
      .drop_duplicates()
...
# Anchor at the Tuesday of the first game week (Tue..Mon NFL week)
first_date = dim_games["game_date"].min().normalize()
# pandas weekday(): Mon=0, Tue=1, ... Sun=6  → subtract days back to the prior/same Tuesday
days_back_to_tuesday = (first_date.weekday() - 1) % 7
first_tuesday = first_date - pd.to_timedelta(days_back_to_tuesday, unit="D")

# Sequential NFL-style week index starting on Tuesday
dim_games["iso_week"] = (
    ((dim_games["game_date"].dt.normalize() - first_tuesday).dt.days // 7) + 1
).astype(int)

# keep your 2024-only season tag
...
fact_player_game["player_name"] = fact_player_game["player_name"].apply(normalize_name)
